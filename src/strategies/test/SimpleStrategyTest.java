...
public class SimpleStrategyTest {
	...
	EnumMap<Products, Double> buy = new EnumMap<Products, Double>(
			Products.class);
	EnumMap<Products, Double> sell = new EnumMap<Products, Double>(
			Products.class);
	...
	/*
	 * @Test public void test_decideAboutSellOffer_should_respond_to_sell_offers_only() { ArrayList<AgentOffer>
	 * firstStageSellOffers = new ArrayList<AgentOffer>(); AgentOffer e = new AgentOffer(offerAgentName, sellOfferPrefix
	 * + "VEGETABLE 12.0 4.0:FRUIT 1.0 5.5:MILK 9.0 8.9:"); firstStageSellOffers.add(e); for (AgentOffer of :
	 * ss.decideAboutSellOffer(firstStageSellOffers)) { assertEquals(1, of.getItemAmount().size()); assertEquals(1,
	 * of.getItemPrice().size()); } }
	 */
	...
	public void test_decideAboutSellOffer_should_return_cheapest_offer_for_single_product_kind() {
		...
		ss.setMyMoney(100);
		ArrayList<AgentOffer> firstStageSellOffers = new ArrayList<AgentOffer>();
		AgentOffer e1 = new AgentOffer(offerAgentName + 1, sellOfferPrefix
				+ "VEGETABLE 12.0 14.0:FRUIT 1.0 5.5:MANURE 9.0 8.9:");
		firstStageSellOffers.add(e1);

		AgentOffer e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "MANURE 12.0 4.0:VEGETABLE 1.0 5.5:FRUIT 9.0 8.9:");
		firstStageSellOffers.add(e2);

		AgentOffer e3 = new AgentOffer(offerAgentName + 3, sellOfferPrefix
				+ "FRUIT 12.0 4.0:MANURE 1.0 5.5:VEGETABLE 9.0 8.9:");
		firstStageSellOffers.add(e3);
		ArrayList<AgentOffer> answers = ss
				.decideAboutSellOffer(firstStageSellOffers);
		...
		for (AgentOffer agentOffer : answers) {
			if (agentOffer.getAgentName().equals(offerAgentName + 2)) {
				...
				assertEquals((Double) 1.0,
						agentOffer.getItemAmount().get(Products.VEGETABLE));
				assertEquals((Double) 5.5,
	...
	public void test_decideAboutSellOffer_should_return_not_bigger_amount_than_he_needs() {
		...
		AgentOffer e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "MANURE 12.0 4.0:VEGETABLE " + (howMuchHeNeeds + 1.0)
				+ " 5.5:FRUIT 9.0 8.9:");
		firstStageSellOffers.add(e2);
		...
		for (AgentOffer agentOffer : answers) {
			if (agentOffer.getAgentName().equals(offerAgentName + 2)) {
				assertEquals(1, agentOffer.getItemAmount().size());
				assertEquals(1, agentOffer.getItemPrice().size());
				assertTrue((Double) (howMuchHeNeeds) >= agentOffer
						.getItemAmount().get(Products.VEGETABLE));
				assertTrue((Double) 5.5 >= agentOffer.getItemPrice().get(
						Products.VEGETABLE));
			} else {
				assertEquals(0, agentOffer.getItemAmount().size());
				assertEquals(0, agentOffer.getItemPrice().size());
	...
	public void test_decideAboutSellOffer_should_return_cheapest_offer_for_multiple_product_kinds_from_different_agents() {
		...
		ArrayList<AgentOffer> firstStageSellOffers = new ArrayList<AgentOffer>();
		AgentOffer e1 = new AgentOffer(offerAgentName + 1, sellOfferPrefix
				+ "VEGETABLE 12.0 14.0:FRUIT 1.0 3.5:MANURE 9.0 8.9:");
		firstStageSellOffers.add(e1);
	...
	public void test_decideAboutSellOffer_should_return_cheapest_offer_for_multiple_product_kinds_from_one_agent() {
		...
		ArrayList<AgentOffer> firstStageSellOffers = new ArrayList<AgentOffer>();
		AgentOffer e1 = new AgentOffer(offerAgentName + 1, sellOfferPrefix
				+ "VEGETABLE 12.0 14.0:FRUIT 10.0 3.5:MANURE 9.0 8.9:");
		firstStageSellOffers.add(e1);

		AgentOffer e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "MANURE 12.0 4.0:VEGETABLE 1.0 5.5:FRUIT 9.0 2.9:");
	...
	public void test_decideAboutSellOffer_should_remember_all_traders() {
		...
		assertTrue(ss.getCurrentWeekBuyOffersHistory().keySet()
				.contains(offerAgentName + 1));
		assertTrue(ss.getCurrentWeekBuyOffersHistory().keySet()
				.contains(offerAgentName + 2));
		assertTrue(ss.getCurrentWeekBuyOffersHistory().keySet()
				.contains(offerAgentName + 3));
	}
	...
	public void test_composeFinalBuyingDecision_should_return_false_for_unknown_offers() {
		...
		AgentOffer e3 = new AgentOffer(offerAgentName + 3, sellOfferPrefix
				+ "FRUIT 12.0 4.0:MANURE 1.0 5.5:VEGETABLE 9.0 8.9:");
		firstStageSellOffers.add(e3);
		Map<String, Boolean> result = ss
				.composeFinalBuyingDecision(firstStageSellOffers);
	...
	public void test_composeFinalBuyingDecision_should_return_true_for_exactly_the_same_offers() {
		...
		e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "VEGETABLE 1.0 5.5:FRUIT 2.0 2.9:");
		secondStageSellOffers.add(e2); // the best one

		e3 = new AgentOffer(offerAgentName + 3, sellOfferPrefix + "");
		secondStageSellOffers.add(e3);
		Map<String, Boolean> result = ss
				.composeFinalBuyingDecision(secondStageSellOffers);
	...
	public void test_composeFinalBuyingDecision_should_return_true_when_getting_cheaper_offer() {
		...
		e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "VEGETABLE 1.0 5.5:FRUIT 2.0 1.9:");
		secondStageSellOffers.add(e2); // the best one
	...
	public void test_composeFinalBuyingDecision_should_return_false_when_getting_worse_offer() {
		...
		e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "VEGETABLE 1.0 5.5:FRUIT 2.0 3.9:");
		secondStageSellOffers.add(e2); // the best one
	...
	public void test_composeFinalBuyingDecision_should_return_true_when_getting_same_price_more_stuff() {
		...
		e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "VEGETABLE 1.0 5.5:FRUIT 3.0 2.9:");
		secondStageSellOffers.add(e2); // the best one
	...
	public void test_decideAboutSellOffer_should_not_buy_more_than_he_can_afford() {
		...
		AgentOffer e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "MANURE 12.0 4.0:VEGETABLE 10.0 5.5:FRUIT 9.0 3.0:");
		firstStageSellOffers.add(e2); // the best one
		...
		for (AgentOffer agentOffer : answers) {
			if (agentOffer.getAgentName().equals(offerAgentName + 2)) {
				assertEquals(2, agentOffer.getItemAmount().size());
				assertEquals(2, agentOffer.getItemPrice().size());
				double totalPaid = agentOffer.getItemAmount().get(
						Products.VEGETABLE)
						* agentOffer.getItemPrice().get(Products.VEGETABLE)
						+ agentOffer.getItemAmount().get(Products.FRUIT)
						* agentOffer.getItemPrice().get(Products.FRUIT);
				assertTrue(
						"buying "
								+ agentOffer.getItemAmount().get(
										Products.VEGETABLE)
								+ " vegetable for "
								+ agentOffer.getItemPrice().get(
										Products.VEGETABLE)
								+ " and "
								+ agentOffer.getItemAmount()
										.get(Products.FRUIT) + " fruit for "
								+ agentOffer.getItemPrice().get(Products.FRUIT)
								+ " paying total: " + totalPaid,
						totalPaid <= myMoney);
	...
	public void test_decideAboutSellOffer_should_remember_in_history_lower_amounts_when_he_doesnt_have_enough_money() {
		...
		ss.decideAboutSellOffer(firstStageSellOffers);
		ArrayList<AgentOffer> bestOffers = ss.getCurrentWeekBuyOffersHistory()
				.get(offerAgentName + 2);
		assertTrue(
				"he remembered: "
						+ bestOffers.get(0).getItemAmount()
								.get(Products.VEGETABLE), bestOffers.get(0)
						.getItemAmount().get(Products.VEGETABLE) <= 8.28);
		assertTrue(
				"he remembered: "
						+ bestOffers.get(0).getItemAmount().get(Products.FRUIT),
				bestOffers.get(0).getItemAmount().get(Products.FRUIT) <= 4.84);
	}
	...
	public void test_composeFinalBuyingDecision_should_agree_for_lower_offers_when_he_reduced_amount_of_stuff() {
		...
		e2 = new AgentOffer(offerAgentName + 2, sellOfferPrefix
				+ "VEGETABLE 8.28 5.5:FRUIT 4.84 3.0:");
		secondStageSellOffers.add(e2); // the best one
	...
	public void test_composeFinalBuyingDecision_should_update_store_money_correctly() {
		...
		buy.put(Products.VEGETABLE, 20.0);
		buy.put(Products.FRUIT, 8.0);
		EnumMap<Products, Double> have = new EnumMap<Products, Double>(
				Products.class);
		...
		AgentOffer e2 = new AgentOffer(bestTraderName, sellOfferPrefix
				+ "MANURE 12.0 4.0:VEGETABLE 10.0 5.5:FRUIT 9.0 3.0:");
		firstStageSellOffers.add(e2); // the best one
		...
		e2 = new AgentOffer(bestTraderName, sellOfferPrefix
				+ "VEGETABLE 8.37 5.5:FRUIT 5.0 3.0:");
	...
	public void test_composeFinalBuyingDecision_should_update_store_buy_and_have_map_correctly() {
		...
		AgentOffer e3 = new AgentOffer(offerAgentName + 3, sellOfferPrefix
				+ "FRUIT 12.0 4.0:MANURE 1.0 5.5:VEGETABLE 9.0 8.9:");
		firstStageSellOffers.add(e3);
		ArrayList<AgentOffer> answers = strag
				.decideAboutSellOffer(firstStageSellOffers);
		double totalPaid = 0.0;
		for (AgentOffer agentOffer : answers) {
		...
		e2 = new AgentOffer(bestTraderName, sellOfferPrefix
				+ "VEGETABLE 8.4 5.5:FRUIT 5.1 3.0:");
		secondStageSellOffers.add(e2); // the best one

		e3 = new AgentOffer(offerAgentName + 3, sellOfferPrefix + "");
		secondStageSellOffers.add(e3);
		Map<String, Boolean> result = strag
				.composeFinalBuyingDecision(secondStageSellOffers);
	...
	// sellers
	// TODO: write test for right amount and so on
	@Test
	public void test_decideAboutSellOffer() {
		sell.put(Products.VEGETABLE, 20.0);
		EnumMap<Products, Double> pricePerItem = new EnumMap<Products, Double>(
				Products.class);
		pricePerItem.put(Products.VEGETABLE, 10.0D);
		ss.setPricePerItem(pricePerItem);
		ss.setSell(sell);
		ArrayList<AgentOffer> buyOffers = new ArrayList<AgentOffer>();

		AgentOffer e1 = new AgentOffer(offerAgentName + 1, buyOfferPrefix
				+ "VEGETABLE 12.0 14.0:");
		buyOffers.add(e1);

		AgentOffer e2 = new AgentOffer(offerAgentName + 2, buyOfferPrefix
				+ "VEGETABLE 10.0 5.5:");
		buyOffers.add(e2); // the best one

		AgentOffer e3 = new AgentOffer(offerAgentName + 3, buyOfferPrefix
				+ "VEGETABLE 9.0 10.0:");
		buyOffers.add(e3);

		ArrayList<AgentOffer> answers = ss.decideAboutBuyOffer(buyOffers);
		assertEquals(3, answers.size());

		assertEquals(answers.get(0).getItemPrice().get(Products.VEGETABLE),
				new Double(14));
		assertEquals(answers.get(1).getItemPrice().get(Products.VEGETABLE),
				null);
		assertEquals(answers.get(2).getItemPrice().get(Products.VEGETABLE),
				new Double(10));
